...
bool GameManager::Init()
{
    ...
    // Initialize and set up constraints
    for (auto& i : mConstraints)
    {
        i.Init(btVector3(0, 0, 0), 0.0f);
    }

    // Bottom wall
    mConstraints[0].Move(7.5f, btVector3(0, 0, 1));
    // Right wall
    mConstraints[1].Move(8.0f, btVector3(1, 0, 0));
    // Top wall
    mConstraints[2].Move(7.5f, btVector3(0, 0, -1));
    // Left wall
    mConstraints[3].Move(8.0f, btVector3(-1, 0, 0));

    // Add objects to renderer and physics world
    for (auto& i : mConstraints)
        mPhysics.AddObject(i.GetBody());

    mRenderer.AddObject(&mPlayer);
    mPhysics.AddObject(mPlayer.GetBody());
...
void GameManager::MainLoop()
{
    ...
    do
    {
        ...
        // Update asteroids and projectiles arrays
        UpdateObjectsArrays();
...
void GameManager::UpdateObjectsArrays()
{
    ...
    if (updateNeeded)
    {
        ...
        mAsteroids.remove_if([](const AsteroidObject& i){ return i.ToDelete(); });
    }
}
...
void GameManager::CheckAsteroids()
{
    ...
    if (mScore >= asteroidsNo)
    {
        ...
        mAsteroids.push_front(AsteroidObject(btVector3(sizeX, sizeY, sizeZ)));
        AsteroidObject* meteor = &mAsteroids.front();
        meteor->Init(btVector3(1, 0, 0), 10.0f);
